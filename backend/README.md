# FastAPI RAG Backend with PostgreSQL + pgvector
...
### 基于权限编码的校验 — `require_permissions`

适用于资源操作，如查看用户列表、管理文件等。

```python
# GET /api/v1/users — 需要 user:read 权限
current_user: User = Depends(require_permissions("user:read"))
```

- 校验当前用户角色是否包含指定的权限编码
- 权限编码通过 `Role ←→ Permission` 多对多关联表分配
- 管理员通过 `init_db.py` 中的 `DEFAULT_ADMIN_PERMISSION_CODES` 获得默认权限

### 基于超级管理员身份的校验 — `is_superuser`
...
- **预置权限**：12 个权限，分为 `user`、`permission`、`rag` 三个模块
- **管理员默认权限**：9 个权限
- **超级管理员**：拥有全部 12 个权限
