...
async def list_users(session: AsyncSession) -> list[User]:
    ...
    该函数返回所有用户列表，按ID升序排列，并预加载用户的角色信息。
...
async def get_user_detail(session: AsyncSession, user_id: int) -> User:
    """
    获取指定用户详情的服务函数

    该函数返回特定用户的信息，包括用户的角色信息。
    如果用户不存在，则抛出异常。
...
async def save_upload(file: UploadFile) -> str:
    ...
    该函数将上传的文件保存到上传目录中，使用UUID生成唯一文件名以避免冲突。
